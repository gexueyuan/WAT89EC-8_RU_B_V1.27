	...
	*/
    ...
    pB->pText = pText;
        ...
        while(1)
        {
            ...
            {
                if(width < GetTextWidth(pParser, pB->pGolScheme->pFont))
                {
                    width = GetTextWidth(pParser, pB->pGolScheme->pFont);
                }
        ...
        pB->textWidth = width;
        pB->textHeight = GetTextHeight(pB->pGolScheme->pFont) * lineCtr;
	#else
		pB->textWidth = GetTextWidth(pText, pB->pGolScheme->pFont);
    	pB->textHeight = GetTextHeight(pB->pGolScheme->pFont);
...
inline void __attribute__((always_inline)) SetButtonTextPosition(BUTTON *button, XCHAR *pCurLine, SHORT lineCtr)
{
    ...
    SetFont(button->pGolScheme->pFont);
    textWidth = GetTextWidth(pCurLine, button->pGolScheme->pFont);

    // check text alignment
    if(GetState(button, BTN_TEXTRIGHT))
    {
        xText = button->right - (textWidth + GOL_EMBOSS_SIZE + 2);
    }
    else if(GetState(button, BTN_TEXTLEFT))
    {
        xText = button->left + GOL_EMBOSS_SIZE + 2;
    }
    else
    {

        // centered	text in x direction
        xText = (button->left + button->right - textWidth) >> 1;
    }

    if(GetState(button, BTN_TEXTTOP))
    {
        yText = button->top + GOL_EMBOSS_SIZE + (lineCtr * GetTextHeight(button->pGolScheme->pFont));
    }
    else if(GetState(button, BTN_TEXTBOTTOM))
    {
        yText = button->bottom - (GOL_EMBOSS_SIZE + button->textHeight) + (lineCtr * GetTextHeight(button->pGolScheme->pFont));
    }
    else
    {

        // centered	text in y direction
        yText = ((button->bottom + button->top - button->textHeight) >> 1) + (lineCtr * GetTextHeight(button->pGolScheme->pFont));
    }
