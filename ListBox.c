...

{
	...
	if (pScheme == NULL)
		pLb->pGolScheme = _pDefaultGolScheme; 
	else 	
		pLb->pGolScheme = (GOL_SCHEME *)pScheme; 	

    pLb->textHeight = GetTextHeight(pLb->pGolScheme->pFont);
...
WORD LbTranslateMsg(LISTBOX *pLb, GOL_MSG *pMsg)
{
    ...
    if(pMsg->type == TYPE_TOUCHSCREEN) {
		...
		if( (pLb->left     < pMsg->param1) &&
	  	    (pLb->right    > pMsg->param1) &&
	   	    (pLb->top      < pMsg->param2) &&
	   	    (pLb->bottom   > pMsg->param2) )
...
LISTITEM* pItem;
    ...
    if(pMsg->type == TYPE_TOUCHSCREEN) {
        ...
        if(pMsg->uiEvent == EVENT_PRESS){
            ...
            pos = (pMsg->param2-pLb->scrollY-pLb->top-LB_INDENT-GOL_EMBOSS_SIZE)/pLb->textHeight;
            pItem = pLb->pItemList;
            while(pos){
                if(pItem == NULL)
                    break;
...
/////////////////////////////////////////////////////////////////////
        case LB_STATE_START:
          	...
          	if (GetState(pLb, LB_HIDE)) {
                if(IsDeviceBusy())
                    return 0;
   	   	        SetColor(pLb->pGolScheme->CommonBkColor);
    	        Bar(pLb->left,pLb->top,pLb->right,pLb->bottom);
    	        return 1;
    	    }    
...
if(GetState(pLb, LB_DRAW)){
            ...
            if(GetState(pLb,LB_DISABLED)){
                temp = pLb->pGolScheme->ColorDisabled;
	        }else{
                temp = pLb->pGolScheme->Color0;
            }

            GOLPanelDraw(pLb->left,pLb->top,pLb->right,pLb->bottom,0,
                         temp,
                         pLb->pGolScheme->EmbossDkColor,
                         pLb->pGolScheme->EmbossLtColor,
                         NULL,
                         GOL_EMBOSS_SIZE);
...
L_LB_DRAW:
            ...
            SetClipRgn(pLb->left+GOL_EMBOSS_SIZE+LB_INDENT,
                       pLb->top+GOL_EMBOSS_SIZE+LB_INDENT,
                       pLb->right-GOL_EMBOSS_SIZE-LB_INDENT,
                       pLb->bottom-GOL_EMBOSS_SIZE-LB_INDENT);

	        SetFont(pLb->pGolScheme->pFont);

            // Set graphics cursor
            MoveTo(pLb->left+GOL_EMBOSS_SIZE+LB_INDENT,
                   pLb->top+GOL_EMBOSS_SIZE+LB_INDENT+pLb->scrollY);
...
/////////////////////////////////////////////////////////////////////
if( GetY() < pLb->bottom-GOL_EMBOSS_SIZE-LB_INDENT )
if((GetY()+ pLb->textHeight) >= (pLb->top+GOL_EMBOSS_SIZE+LB_INDENT)){
        ...
        case LB_STATE_ERASEITEM:
            ...
            if( GetState(pLb, LB_DISABLED)){
                SetColor(pLb->pGolScheme->ColorDisabled);
            }else{
                SetColor(pLb->pGolScheme->Color0);
                if(pCurItem != NULL)
                if(pCurItem->status&LB_STS_SELECTED){
                    SetColor(pLb->pGolScheme->Color1);
                }
            }

            Bar(pLb->left+GOL_EMBOSS_SIZE+LB_INDENT,
                GetY(),
                pLb->right-GOL_EMBOSS_SIZE-LB_INDENT,
                GetY()+ pLb->textHeight);
            ...
            if (!GetState(pLb, EB_CENTER_ALIGN|EB_RIGHT_ALIGN)) {
                MoveTo(pLb->left+GOL_EMBOSS_SIZE+LB_INDENT, GetY());
            }else{
                temp = GetTextWidth(pCurItem->pText, pLb->pGolScheme->pFont);
                if(pCurItem->pBitmap != NULL)
                {
                    temp += GetImageWidth(pCurItem->pBitmap)+LB_INDENT;
                }
                if (GetState(pLb, EB_RIGHT_ALIGN)) {
			        MoveTo(pLb->right-temp-LB_INDENT-GOL_EMBOSS_SIZE, GetY());
                }else{
                    MoveTo((pLb->left+pLb->right-temp)>>1, GetY());
                }
            }

            if( GetState(pLb, LB_DISABLED)){
                SetColor(pLb->pGolScheme->TextColorDisabled);
            }else{
                if(pCurItem->status&LB_STS_SELECTED){
                    SetColor(pLb->pGolScheme->TextColor1);
                }else{
                    SetColor(pLb->pGolScheme->TextColor0);
                }
            }
...
L_LB_NEXTITEM:
        ...
        case LB_STATE_ITEMFOCUS:
            ...
            if(pLb->pFocusItem == pCurItem){
                ...
                temp = GetY();
                Rectangle(pLb->left+GOL_EMBOSS_SIZE+LB_INDENT,
                          GetY(),
                          pLb->right-GOL_EMBOSS_SIZE-LB_INDENT,
                ...
                // Scroll if needed
                if(GetY() < (pLb->top+GOL_EMBOSS_SIZE+LB_INDENT)){
                    pLb->scrollY += (pLb->top+GOL_EMBOSS_SIZE+LB_INDENT)-GetY();
                    SetState(pLb, LB_DRAW);
                    goto L_LB_DRAW;
                }
                if((GetY()+pLb->textHeight) > (pLb->bottom-GOL_EMBOSS_SIZE-LB_INDENT)){
                    pLb->scrollY += pLb->bottom-GetY()-pLb->textHeight-GOL_EMBOSS_SIZE-LB_INDENT;                 
                    SetState(pLb, LB_DRAW);
                    goto L_LB_DRAW;
                }
            }
