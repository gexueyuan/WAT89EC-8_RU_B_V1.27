				  ...
				  XCHAR *pText, GOL_SCHEME *pScheme)				 
{
	...
	pMtr->ID      	= ID;					// unique id assigned for referencing
	pMtr->pNxtObj 	= NULL;					// initialize pointer to NULL
	pMtr->type    	= OBJ_METER;	 	    // set object type
	pMtr->left      = left;    	  	    	// left,top coordinate
	pMtr->top       = top;    	  	    	// 
	pMtr->right     = right;   	  	    	// right,bottom coordinate
	pMtr->bottom    = bottom;  	  	    	// 
	pMtr->range		= range;
	pMtr->value		= value;
    pMtr->state   	= state; 	            // state
    pMtr->pText 	= pText;
	...
	if (pScheme == NULL)
		pMtr->pGolScheme = _pDefaultGolScheme; 
	else 	
		pMtr->pGolScheme = (GOL_SCHEME *)pScheme; 	
    ...
    switch(state) {
    	case IDLE:
			...
			if (GetState(pMtr,MTR_HIDE)) {  				      	// Hide the meter (remove from screen)
		    	SetColor(pMtr->pGolScheme->CommonBkColor);
		       	Bar(pMtr->left, pMtr->top,pMtr->right, pMtr->bottom);
			    return 1;
			}
			...
			if (GetState(pMtr,MTR_DRAW)) {  				      
				...
				GOLPanelDraw(pMtr->left, pMtr->top, pMtr->right, pMtr->bottom, 0, 
							 pMtr->pGolScheme->Color0, 
							 pMtr->pGolScheme->EmbossLtColor, 
							 pMtr->pGolScheme->EmbossDkColor, 
        ...
        case TEXT_DRAW:	
			...
			SetColor(pMtr->pGolScheme->TextColor1);
			SetFont(pMtr->pGolScheme->pFont);
			temp = GetTextWidth(pMtr->pText, pMtr->pGolScheme->pFont);
			
			// set the start location of the meter title
			#if defined (MTR_WHOLE_TYPE) 
		
				MoveTo( pMtr->xCenter-(temp>>1), pMtr->yCenter+pMtr->radius+GetTextHeight(pMtr->pGolScheme->pFont));
		...
		case ARC2_DRAW:
			...
			SetColor(pMtr->pGolScheme->Color1);
			i = DEGREE_END;
			state = SCALE_COMPUTE;
...
mtr_scale_compute:		
		...
		case SCALE_DRAW:
			...
			if ((i%45) == 0) {
				...
				x2 = GetTextWidth((&strVal[SCALECHARCOUNT-j]), pMtr->pGolScheme->pFont);
				y2 = GetTextHeight(pMtr->pGolScheme->pFont);
...
mtr_needle_draw_st:
			...
			if (GetState(pMtr,MTR_DRAW_UPDATE)) {  				      
				...
				SetColor(pMtr->pGolScheme->Color0);
		...
		case VALUE_ERASE:
			...
			temp = GetTextWidth(tempXchar, pMtr->pGolScheme->pFont);
			...
			SetColor(pMtr->pGolScheme->Color0);
				...
				Bar(    pMtr->xCenter-(temp>>1),
				    	...
				    	pMtr->xCenter+(temp>>1),
		 				pMtr->yCenter+pMtr->radius+GetTextHeight(pMtr->pGolScheme->pFont));
		...
		case VALUE_DRAW:
			...
			temp = GetTextWidth(&strVal[SCALECHARCOUNT-j], pMtr->pGolScheme->pFont);
